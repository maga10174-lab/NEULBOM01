...
const MediaEditForm: React.FC<{
  ...
  onSave: (data: (MediaFormState & {file?: File}) | (MediaFormState & {file?: File})[]) => Promise<void>;
  onCancel: () => void;
  ...
  const [stagedFiles, setStagedFiles] = useState<{url: string; name: string; file: File}[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (stagedFiles.length > 0) {
        const itemsToSave = stagedFiles.map(file => ({
          type: 'image' as const,
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        const filePromises = Array.from(files).map(async (file: File) => {
            ...
            return { url: base64, name: file.name, file: file };
        });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        <button type="submit" disabled={isSaving} className="rounded-md border border-transparent bg-primary-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-primary-700 disabled:bg-gray-400">
            {isSaving ? "저장 중..." : "저장"}
        </button>
      </div>
    </form>
...
export const GalleryManagement: React.FC<{
  ...
  addItems: (items: (Omit<GalleryMediaItem, 'id' | 'order'> & { file?: File })[]) => Promise<void>;
  updateItem: (itemId: string, data: Partial<GalleryMediaItem>, newFile?: File) => Promise<void>;
  deleteItems: (itemIds: string[]) => Promise<void>;
  reorderItems: (newList: GalleryMediaItem[]) => Promise<void>;
}> = ({ galleryMedia, addItems, updateItem, deleteItems, reorderItems }) => {
  const [editingItem, setEditingItem] = useState<GalleryMediaItem | null>(null);
  const [isAddingNew, setIsAddingNew] = useState(false);
  const [selectedItems, setSelectedItems] = useState<Set<string>>(new Set());

  const handleSave = async (data: (MediaFormState & {file?: File}) | (MediaFormState & {file?: File})[]) => {
    if (Array.isArray(data)) {
      const itemsToAdd = data.map(d => {
        const { file, ...rest } = d;
        const baseItem: Omit<GalleryMediaItem, 'id' | 'order'> & { file?: File } = { ...rest };
        if (file) {
          baseItem.file = file;
        }
        return baseItem;
      ...
      const updatedData: Partial<GalleryMediaItem> = rest;
      await updateItem(editingItem.id, updatedData, file);
    } else {
      const { file, ...rest } = data;
      const newItem: Omit<GalleryMediaItem, 'id' | 'order'> & { file?: File } = { ...rest };
      if (file) {
        newItem.file = file;
      }
